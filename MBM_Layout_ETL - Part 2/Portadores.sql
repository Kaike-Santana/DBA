...
--Alter Column Id_Empresa Int Null
--
--CREATE UNIQUE NONCLUSTERED INDEX UQ_cod_portador_id_empresa_grupo
--ON portadores (cod_portador, id_empresa_grupo);


--select *
--,	rw = row_number() over (partition by id_empresa_grupo, cod_portador order by incl_data asc)
--from portadores


Declare @vEmpresa Int = (Select Id_Empresa_Grupo From Empresas_Grupos Where Nome = 'POLIREX')
...
Select 
  [id_empresa]			=  Bancos_Contas.id_empresa
, [id_empresa_grupo]	=  (Select Id_Empresa_Grupo From Empresas_Grupos Where Nome = 'Polirex')
...
outer apply (
	...
	,	id_empresa
    from bancos_contas
    where bancos_contas.nro_conta   = fato.nro_conta   collate latin1_general_ci_ai
      and bancos_contas.nro_agencia = fato.nro_agencia collate latin1_general_ci_ai
      and bancos_contas.cod_banco   = fato.cod_banco   collate latin1_general_ci_ai
    order by id_banco_conta -- você pode ajustar o critério de ordenação se necessário
) Bancos_Contas
where not exists (
				   select *
				   from Portadores dim
				   where dim.cod_portador   = fato.cod_portador Collate latin1_general_ci_ai
