...
SELECT  '__export__.mmp_pre_dossie_' || DOS.id AS ID
			...
			,CASE WHEN PAR.data_vencimento = CURRENT_DATE  
					  OR PAR.data_vencimento = CURRENT_DATE + 1 
					  OR PAR.data_vencimento = CURRENT_DATE + 2
					  OR PAR.data_vencimento = CURRENT_DATE + 3
					  OR PAR.data_vencimento = CURRENT_DATE + 4 	 	
					THEN 'Preventivo' 
					ELSE 'Acordo em Dia' 
			 END AS situ_acordo
...
SELECT * FROM TEMP_INCLUSAO_TELEFONE

SELECT 
			'55' || TELEFONE || ',' || AUTOR AS WHATS		
