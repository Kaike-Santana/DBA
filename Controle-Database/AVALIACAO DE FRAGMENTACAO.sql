SELECT db_name(a.database_id) AS "BANCO"
   ...
   ,'ALTER TABLE ' + QUOTENAME(ss.name) + '.' + QUOTENAME(object_name(a.object_id)) + ' REBUILD;' AS "REBUILD TABELA"  -- TABELA SEM ÍNDICE CLUSTERIZADO
   ,'ALTER INDEX ' + QUOTENAME(si.name) + ' on ' + QUOTENAME(ss.name) + '.' + QUOTENAME(object_name(a.object_id)) + ' REBUILD;' AS "REBUILD INDICE" -- É fornecido o comando para desfragmentar o índice individualmente
   ,*
